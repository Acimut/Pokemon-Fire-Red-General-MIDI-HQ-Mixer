...
void FadeOutBody_bl(struct MusicPlayerInfo *mplayInfo)
{
    FadeOutBody(mplayInfo);
}

void ClearChain_bl(void *x)
{
    ClearChain(x);
}

void Clear64byte_bl(void *addr)
{
    Clear64byte(addr);
}

void TrkVolPitSet_bl(struct MusicPlayerInfo *mplayInfo, struct MusicPlayerTrack *track)
{
...
u32 MidiKeyToFreq_bl(struct WaveData *wav, u8 key, u8 fineAdjust)
{
    return MidiKeyToFreq(wav, key, fineAdjust);
}
